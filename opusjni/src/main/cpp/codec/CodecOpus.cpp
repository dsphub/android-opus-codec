...
int CodecOpus::decoderCreate(opus_int32 Fs, int numChannels) {
    if (numChannels != 1 && numChannels != 2)
        LOGE(TAG,
             "[decoderCreate] numChannels is incorrect: %d - it must be either 1 or 2, otherwise the encoder may works incorrectly",
             numChannels);

    int ret;
    decoder = opus_decoder_create(Fs, numChannels, &ret);

    if (ret < 0) {
        LOGE(TAG, "[decoderCreate] couldn't create decoder; ret: %d; error: %s", ret,
             opus_strerror(ret));
        free(decoder);
        return -1;
    } else {
        LOGD(TAG, "[decoderCreate] decoder successfully initialized");
...
std::vector<uint8_t> CodecOpus::decodeFloat(uint8_t *data, int length, int frameSize) {
    opus_int8 *outBuffer = (opus_int8 *) malloc(sizeof(opus_int8) * 1024);
    int resultLength = opus_decode_float(decoder, (unsigned char *) outBuffer, (opus_int32) length, pcm,
                                         frameSize, 0);  //TODO pcm type
    if (resultLength < 0) {
    ...
    for (int i = 0; i < resultLength; ++i) {
        uint8_t hi = (uint8_t) ((*outBuffer)[i]);
        result.push_back(hi);
    }
    return result;
}
